...
def removeSpecialChars(stringname):
    stringoutput=''.join(e for e in stringname if e.isalnum())
    return stringoutput.lower()

def validateStorageAccount(storageaccountame):
    stroage_account=removeSpecialChars(storageaccountame)
    if(len(stroage_account) <= 19):
        return True

@router.put("/repos/{org}/{repo}/functionapps")
def deploy_functionapp_app(org: str, repo: str, env: Environment, cred: Credentials, accept:Optional[str] = 'application/vnd.github.luke-cage-preview+json'):
    if(validateStorageAccount(repo+env.environment)):
        ...
        file_name = "create_function_app_workflow.yaml"
        create_file(org,repo,template_path,file_name,destination_path)
        time.sleep(3)
        url = "https://api.github.com/repos/{}/{}/dispatches".format(org,repo)
        response = requests.post(url, json={"event_type": "create_function_app", "client_payload": {**env.dict(),**cred.dict(),"repo":removeSpecialChars(repo),"storage_account_name":removeSpecialChars(repo+env.environment)}}, headers={'Accept':accept, 'Authorization': AUTH_TOKEN})
        if response.ok:
            return JSONResponse(status_code=200, content={"functionapp name": "alm-app-"+env.app_type+"-"+removeSpecialChars(repo)+"-"+env.environment})
        else:
            return(response.json())
    else:
         return ({"message": "The Combination of Repository and Environment cannot exceed 19 characters excluding special characters"})



@router.delete("/repos/{org}/{repo}/functionapps/{functionapp_name}")
def destroy_functionapp_app(org: str, repo: str, functionapp_name: str, resource_group: str, cred: Credentials, accept:Optional[str] = 'application/vnd.github.luke-cage-preview+json'):
    create_update_github_repo_secret(org,repo,"AZURE_CLIENT_ID",cred.client_id)
    create_update_github_repo_secret(org,repo,"AZURE_CLIENT_SECRET",cred.client_secret)
    create_update_github_repo_secret(org,repo,"AZURE_SUBSCRIPTION",cred.subscription)
    ...
    template_path = "action_templates/"
    destination_path = ".github/workflows/"
    file_name = "delete_function_app_workflow.yaml"
    create_file(org,repo,template_path,file_name,destination_path)
    url = "https://api.github.com/repos/{}/{}/dispatches".format(org,repo)
    response = requests.post(url, json={"event_type": "delete_functionapp_app", "client_payload": {**cred.dict(),"functionapp_name":functionapp_name,"resource_group":resource_group}}, headers={'Accept':accept, 'Authorization': AUTH_TOKEN})
    return(response)

@router.get("/repos/{owner}/{repo}/functionapps")
def get_functionapp_app(owner: str, repo:str, accept:Optional[str] = 'Accept: application/vnd.github.3.raw'):
    file_path=".github/functions/functions.json"
